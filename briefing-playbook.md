...
| cron 日志只有 header 无产出 | `run-YYYY-MM-DD.log` 只有开始行、`ps` 无 run-briefing 进程、lock 文件为空 → cron 启动即失败（8/10、8/17、8/23、8/27、8/28、8/29、8/30、8/31 八次实测，间歇性复发），可放心手动执行；手动 scp 会覆盖 cron 留下的同名空文件，推送后再核一次 MD5 即可 |
...
| QA 报 "PLAIN HAS DIGITS" | 引入段混入 "2 纳米"、"16 岁"、"17 岁"、"113 天"、"8 万美元"、"htmx 4.0"、"Python 2/3"、游戏名里的 ASCII 数字（如"《半条命 2》"，9/1 实测）等写法即触发（8/26、8/30、9/1 实测）；改写为不含数字的表述（"最新工艺""未成年人""重新站上八万美元""新的大版本""旧版 Python 迁到新版""《半条命》系列第三章"），具体数字与版本号全部留给 card；中文数字（如"四成多"）可通过检查但仍尽量避免 |
