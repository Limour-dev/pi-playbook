...
- 快讯类 feed（金十数据、联合早报、风向旗、竹新社）：**看标题即可**，偶尔读正文。金十每天有大量模板噪音（`https://www.jin10.com/`、「金十图示」「期货热点追踪」「A股温度计」「XX价格追踪」「VIP事件提醒」「十分钟后公布」等），扫标题时直接跳过；真正有信息量的是带具体数字的快讯与「金十数据整理：每日科技要闻速递 / 中东局势跟踪 / 俄乌冲突跟踪」，这三类建议读正文（一条顶十条）。三类的发布时点固定：中东/俄乌跟踪约 05:30–05:50 发布**当天**内容，科技要闻速递约次日早 07:30 发布**前一日**内容——所以 06:00 执行时拿到的最新一期速递仍是前一天的（实测 2026-09-16：中东、俄乌跟踪是 9 月 16 日版，科技速递最新只到 9 月 14 日版），不要把「科技速递日期比跟踪早一天」当成漏抓。**周末例外**：周六与周日的三类综述发布明显更晚甚至当天整期不出，06:00 执行时窗口内最新一期仍是前一天的（实测：2026-09-12 周六 09:15/09:20/09:31、2026-09-13 周日 08:38/08:42/08:53，2026-09-14 周一恢复到 05:31；2026-09-19 周六 06:00 执行时窗口内最新一期仍是 9-18 的 05:23 中东跟踪与 07:37 科技速递；**2026-09-20 周日 06:00 窗口里最新一期中东跟踪是 09-19 09:40 才发的「9 月 18 日」版、科技速递整期缺失**，与「周末发布更晚」一致）——这不算抓取失败，别写成漏抓，直接用已有的最新一期加当天的即时快讯成稿。注意金十/联早的 feed 正文有时只有导语（实测联早特稿 297686 的 `content` 只有开头一段，`--plain-text` 也拿不到全文），这类深度稿按导语 + 标题成稿，不要为补全而编造。
...
- **昨日终点之前的漏标 unread 要过一遍内容，不要只当待标 id**：这些条目虽不属于「新增素材」，但可能包含上一轮没来得及覆盖的实质内容——实测 2026-09-19 有 30 条落在 9-18 12:55 之前的 unread，其中 6 篇是 Nature 当天 08:00 那批（首例 ALS 的 RNA 疗法、大脑发育新实验、欧洲太空独立等），昨日简报完全没写；另有十余篇十年之约博客。做法：把漏标 id 里属于 Nature / MIT 科技评论 / 深度博客的条目读一遍，用昨日简报正文 grep 关键词确认未覆盖后补写进当天简报，其余（金十快讯、联早即时）只标记已读即可。
...
| 正文里的具体数字／细节无法溯源 | 用「全窗口正文语料 grep」判定：`miniflux entries --status read,unread --after <起点> --before <终点> --limit 200 --compact --plain-text` 全量分页拉正文（两天窗 1688 条约 9 次请求、一周窗 3070 条约 16 次，几秒钟），落盘后本地 grep 关键词。**grep 关键词用最短的可辨识短语（搜「英国央行」而不是整句），并把两天窗与一周窗两份语料合并后再判**——实测 2026-09-18 有一次核对把「英国央行连续第六次维持 3.75% 不变」误判成无出处，其实原文就在 296520、296344 里。确实搜不到才改写为可验证的等价表述。注意 `--compact` 单独用时 `content` 字段为空（§9.1 的 dump 因此不含正文），拉正文必须加 `--plain-text` |
| 用 `--plain-text` 语料核事实时把「机构前瞻」当成结果 | 决议／数据类事实只采信**发布时点晚于事件本身**的报道，并检查措辞里有没有「预计／料／前瞻／机构预测」。实测 2026-09-18：296007（9-17 13:35，决议前）写的是荷兰国际「预计英国央行以 6 比 3 维持 3.75% 不变」，把它当结果写成「投票 6 比 3」就是错的；决议结果见 296520、296344 的「连续第六次会议将政策利率维持在 3.75% 不变」。规则：前瞻稿只能写成「市场预期」，投票比例、决议措辞这类细节只在决议后的报道里取；改完 HTML 要重跑 §9.4 并重推、重核两端 MD5 |
| 手动会话写到一半，`run-briefing.sh` 起来了并接管成品 | 实测 2026-09-18：手动会话 13:37 开始、约 13:51 写出 HTML（footer 仍是 `__MARKED__`、未推送），13:44 启动的 `run-briefing.sh` 把它当成「被中断的同日成品」，校验补完（改 footer、更新 HN 分数）后推送并提交了 playbook。**此时手动会话不要抢推**：先 `lsof briefing-playbook/run-$(date +%F).log` 确认那个 run 已结束（无持有者），再核两端 MD5，**并且必须逐句复核它改过的地方**（本次它改了 3 处，其中「投票 6 比 3」是前瞻数据，已改回）。若自己先推了，`scp` 会把对方更新的分数覆盖回去，需要重推一次并保持 `index.html` 指向最新 |
| 同一订阅事件跨天状态反转 / 连续剧式进展 | 写作前先读昨日简报对应段落，反转写成「同一事件的最新一轮交锋」并并列双方说法；连续剧事件围绕新角度展开，不复述昨天内容。**判定「是否已写过」用全文 grep 昨日 HTML 的关键词**（不要凭印象），实测 2026-09-20：Anthropic 湿实验室昨天已由路透稿写过一条，今天的 MIT 中文稿新增的是负责人确认与蛋白设计命中率（15 个靶点成功 14 个、22.6%/26.7% 对比传统 10%–15%），于是写成「新披露的是它想解决的速度差」；同一天 Anthropic 的 IPO 也从昨天的「推迟至 11 月」变成「推向中期选举之后」，按最新一轮口径写即可 |
