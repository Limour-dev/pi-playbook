...
| cron 日志只有 header 无产出 | `run-YYYY-MM-DD.log` 只有开始行、`ps` 无 run-briefing 进程、lock 文件为空 → cron 启动即失败（8/10、8/17、8/23、8/27、8/28、8/29、8/30、8/31、9/2、9/4、9/5 十一次实测，间歇性复发），可放心手动执行；手动 scp 会覆盖 cron 留下的同名空文件，推送后再核一次 MD5 即可。**新变体（9/6 第十二次）**：日志不是只有 header，而是整段"你贴出了 briefing-playbook.md，但没有说明要做什么…"的提问式输出（agent 启动成功但把 `-p @file` 当成"用户贴手册未给任务"、没按 §0 执行就退出 exit=0）——同样无产出，按未产出处理手动执行；判定时以"日志里有没有执行痕迹/成品文件"为准，别被非空日志骗了 |
